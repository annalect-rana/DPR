...
#' @param searchgateway_sub These are the sub channels which are determined to belong as a gateway event, usually brand search. These are treated differently to normal events. must be supplied as a character vector.
#' @param coefs_out This defaults to FALSE, but if TRUE will return the coefficients fitted by the process.
#' 
...
blue_function <- function(username, DPID, file_save_location, cost_file_location, output_dir,
                          click_cutoff = 5, imp_cutoff=250, revenue_per_conv=0,
                          revenue_option=c(
                            "revenue_per_conv",
                            "revenue_in_costs",
                            "first_conversion_revenue",
                            "all_conversion_revenue"), 
                          extra_csv_outputs=TRUE,
                          searchgateway_sub=NULL, coefs_out=FALSE) {
  start.time <- Sys.time()    
  analyst <- username
  message("Loading Files")
  revenue_option <- match.arg(revenue_option)
  ...
  load(file = file.path(file_save_location,paste0(analyst,"_",DPID,"_icM_conv_sec_filt.RData")))
  ...
  costs <- read.csv(cost_file_location)# now moved to the top for checking
  if(revenue_option=="revenue_in_costs"&(!("revenue"%in% colnames(costs)))) {
    stop("there is no column 'revenue' in the cost file, and you've selected the revenue_option 'revenue_in_costs'")
  }
  ...
  keepers_1 <- names(which(colSums(M_red_1_conv[,-c(1,ncol(M_red_1_conv)),drop=FALSE])!=0))
  keepers_2 <- names(which(colSums(M_red_2_conv[,-c(1,ncol(M_red_2_conv)),drop=FALSE])!=0))

# remove those which are in searchgateway_sub - they get their own small subchaggr
  if(!is.null(searchgateway_sub) & any(paste0(searchgateway_sub,"_2") %in% keepers_2)) {
    keepers_2 <- keepers_2[-which(keepers_2%in% paste0(searchgateway_sub,"_2"))]
    }
  ...
  subChInd_small_1 <- names(which(
    colSums(M_red_1[,keepers_1,drop=FALSE]>0) < cutoff))
  ...
  if(!is.null(searchgateway_sub)){
  form <- as.formula(paste("conv ~", paste(names(M_red)[!(names(M_red) %in% c("user_id", "conv",paste0(searchgateway_sub,"_2")))], collapse = " + ")))
  }
  ...
  if (!is.null(searchgateway_sub)){
    searchgateway_sub <- paste0(searchgateway_sub,"_2")
    coef[searchgateway_sub] <- 0.0000000001
  }
  ...
  icM_conv <- unique(icM_conv_sec_filt)
  ...
  sum_wt <- aggregate(weight~user_id,icM_conv,sum)
  colnames(sum_wt) <- c("user_id","tot_wt")
  icM_conv <- merge(icM_conv,sum_wt)
  icM_conv$weight <- icM_conv$weight/icM_conv$tot_wt
  ...
  icM_conv$week             <- gsub("-W","",ISOweek(icM_conv$time))
  icM_conv$month            <- paste0(icM_conv$record_year,str_pad(icM_conv$record_month,width = 2,pad = "0"))
  
  # multiply revenues by the weight
  if(revenue_option == "first_conversion_revenue") {
    icM_conv$revenue <- icM_conv$initial_revenue*icM_conv$weight
  }
  if(revenue_option == "all_conversion_revenue") {
    icM_conv$revenue <- icM_conv$all_revenue*icM_conv$weight
  }
  if(revenue_option == "revenue_per_conv") {
    icM_conv$revenue <- revenue_per_conv*icM_conv$weight
  }
  if(revenue_option == "revenue_in_costs") {
    icM_conv$revenue <- 0
  }


  
  icM_conv$all_revenue <- icM_conv$all_revenue*icM_conv$weight
  ...
  ### calc exclusive conversions from the icM_conv - allows for weekly, monthly and topline tab1
  {
    ...
    icM_conv$conv_week <- gsub("-W","",ISOweek(icM_conv$conv_date))
    ...
    revenue <- list(total_sub    = aggregate(revenue~sub_channel+channel,
                                             icM_conv,FUN=sum,na.rm=T),
                    total_chan   = aggregate(revenue~channel,
                                             icM_conv,FUN=sum,na.rm=T),
                    monthly_sub  = aggregate(revenue~sub_channel+channel+conv_month,
                                             icM_conv,FUN=sum,na.rm=T),
                    monthly_chan = aggregate(revenue~channel+conv_month,
                                             icM_conv,FUN=sum,na.rm=T),
                    weekly_sub   = aggregate(revenue~channel+sub_channel+conv_week,
                                             icM_conv,FUN=sum,na.rm=T),
                    weekly_chan  = aggregate(revenue~channel+conv_week,
                                             icM_conv,FUN=sum,na.rm=T))
    
    
    revenue$total_sub$level    <- "1_s"
    revenue$total_chan$level   <- "1_c"
    revenue$monthly_sub$level  <- "2_s"
    revenue$monthly_chan$level <- "2_c"
    revenue$weekly_sub$level   <- "3_s"
    revenue$weekly_chan$level  <- "3_c"
    
    revenue <- do.call(rbind.fill,revenue)
    ...
    colnames(revenue)[which(colnames(revenue)=="conv_month")] <- "record_month"
    colnames(revenue)[which(colnames(revenue)=="conv_week")] <- "record_week"
  }
  ...
  costs$record_month <- paste0(costs$record_year,
                               str_pad(month(costs$date),
                                       width = 2,pad = "0"))
  costs$record_week <- gsub("-W","",ISOweek(costs$date))
  ...
  if(revenue_option == "revenue_in_costs") {
    ...
    colnames(costs2) <- c("channel","sub_channel","record_month",
      ...
      costs2  <- costs2[,c("channel","sub_channel","record_month",
                       "record_week","clicks","impressions","media_cost","level")]
      colnames(costs2) <- c("channel","sub_channel","record_month",
                           "record_week","cost_clicks","cost_impressions","media_cost","level")
    }
  ...
  tab2_sub <- aggregate(cbind(weight,revenue)~channel+sub_channel+conv_date,
                        icM_conv,FUN=sum)
  tab2_chan <- aggregate(cbind(weight,revenue)~channel+conv_date,
                        icM_conv,FUN=sum)
  tab2_sub$level  <- "s"
  tab2_chan$level <- "c"
  ...
  costs3 <- list(channel=aggregate(cbind(clicks,impressions,media_cost,revenue)~channel+date,
                                   costs,FUN=sum,na.rm=T),
                 sub_channel=aggregate(cbind(clicks,impressions,media_cost,revenue)~channel+sub_channel+date,
                                       costs,FUN=sum,na.rm=T))
  costs3$channel$level <- "c"
  costs3$sub_channel$level <- "s"
  
  costs3 <- rbind.fill(costs3)
  
  if(revenue_option!="revenue_in_costs") {
    tab2 <- merge(tab2,subset(costs3,select=c("date","sub_channel","channel","media_cost","level")),
                  by=c("date","sub_channel","channel","level"),all=T)
  } else {
    tab2 <- merge(tab2[,-which(colnames(tab2)=="revenue")],subset(costs3,select=c("date","sub_channel","channel","media_cost","level","revenue")),
                  by=c("date","sub_channel","channel","level"),all=T)
  }
  ...
  #need to make pathChanDf again, not deduping this time
{
  ...
  ## Sub Channel
  setkey(pathDt,channel_path,sub_channel_path)
  ...
  tab4 <- rbind.fill(conv_rows_by_len,by_channel_conv_rows_by_len)
  colnames(tab4)[which(colnames(tab4)=="chan")] <- "channel"
...
setnames(lc_na,"channel","lc_channel")
setnames(lc_na,"sub_channel","lc_sub_channel")

icM_conv <- merge(icM_conv,
                   lc_na[,c("user_id","lc_channel","lc_sub_channel"),],
                   all.x=T)
...
#     return(x)}))
  ...
  icM_conv <- icM_conv[order(icM_conv$user_id,
                             icM_conv$time,decreasing=T),]
  ...
  na_os$hour_of_day <- hour(na_os$time)
  na_os$day_of_week <- wday(na_os$time)-1
  na_os$conversions <- "C"
  na_os$daypart <- daypart_match$daypart[match(na_os$hour_of_day,
                                                         daypart_match$hour_of_day)]
  ...
  tab7_list <- list(
    ...
    ic_dayp = aggregate(cbind(impressions,clicks)~
                          ...
                          os_id,
                         data=event_dow_distr,FUN=sum),
    conv_hod  = aggregate(conversions~os_id+day_of_week+hour_of_day,
                          data=na_os,FUN=length),
    conv_dayp = aggregate(conversions~os_id+day_of_week+daypart,
                          data=na_os,FUN=length))
...
tab7$day_of_week <- day_match$dow[match(tab7$day_of_week,day_match$day_of_week)] 
  ...
  write.csv(tab4,   file.path(output_dir,'tab_4.csv'),  row.names = F)
...
write.csv(M_red,file=file.path(output_dir,'M_red.csv'))
 ...
 # if requested output the model coefs
  
if(coefs_out){write.csv(coef,file=file.path(output_dir,'coefs.csv'), row.names = TRUE)}

end.time <- Sys.time()
  function_summary <- cat(paste0("Blue function Runtime Stats:"),
