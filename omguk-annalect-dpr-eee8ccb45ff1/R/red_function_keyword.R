...
#' @examples
#' 
#' #red_function_keyword(username="tim_scholtes",pw="",
...
red_function_keyword <- function(username,pw,DPID,dbname="annalectuk",dbHost="54.77.118.78",
                         ...
                         ppc_match=FALSE,
                         ppc_match_col=c("paid_search_campaign",
...
failure <- checks_function_keyword(startDate,endDate,
                                   startPre,endPre,
                                   reupload_hierarchy,
                                   ppc_match,
                                   hierarchy_location,
...
if(length(tables_to_remove)>0) {
  dbSendQuery(redshift_con,paste0("DROP TABLE IF EXISTS ds_",
                                  agency,".",tables_to_remove,";",
                                  collapse=" \n"))
}
  ...
  dbSendQuery(redshift_con,paste0("
    DROP TABLE IF EXISTS ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_pre_na_UIDs;
    create table ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_pre_na_UIDs as
    select user_id
    from ds_",agency,".dc2_na_",networkId,"
  ...
  dbSendQuery(redshift_con,paste0("
    DROP TABLE IF EXISTS ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na;
    ...
    placement_id, campaign_id, site_id, advertiser_id, record_month, cast(concat(to_char(min(s.event_time), 'iyyy'),to_char(min(s.event_time), 'iw')) as INTEGER) as record_week, record_year,record_date, total_revenue as initial_revenue
    from ds_",agency,".dc2_na_",networkId," as s
...
group by user_id,  activity_id, os_id,browser_id, paid_search_legacy_keyword_id,
    placement_id, campaign_id, site_id, advertiser_id, record_month, record_week, record_year,record_date, total_revenue
    "))
  

# calc for the revenue across all conversion types now:
 dbSendQuery(redshift_con,paste0("
    DROP TABLE IF EXISTS ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_allrevenue;
...
group by user_id"))
  ...
  dbSendQuery(redshift_con,paste0("
  DROP TABLE if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_c;
  ...
  advertiser_id, record_month, cast(concat(to_char(event_time, 'iyyy'),to_char(event_time, 'iw')) as INTEGER) as record_week, record_year,record_date,segment_value_1 as paid_search_legacy_keyword_id
  FROM ds_",agency,".dc2_c_",networkId," as c1
  ...
  and user_id not in (
    select e.user_id from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_pre_na_UIDs as e)
  UNION ALL
  SELECT c.user_id, c.event_time as time, c.os_id,c.city_id, c.placement_id,
  c.campaign_id, c.site_id,c.browser_platform_id as browser_id,
  c.advertiser_id, c.record_month,
  cast(concat(to_char(c.event_time, 'iyyy'),to_char(c.event_time, 'iw')) as INTEGER) as record_week, c.record_year,c.record_date,c.segment_value_1 as paid_search_legacy_keyword_id
  FROM ds_",agency,".dc2_c_",networkId," as c
  ...
  dbSendQuery(redshift_con,paste0("
    DROP TABLE if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_i;
    ...
    advertiser_id, record_month,
     cast(concat(to_char(event_time, 'iyyy'),to_char(event_time, 'iw')) as INTEGER) as record_week, record_year,record_date
    FROM ds_",agency,".dc2_i_",networkId," as i1
    ...
    cast(concat(to_char(i.event_time, 'iyyy'),to_char(i.event_time, 'iw')) as INTEGER) as record_week, i.record_year,i.record_date
    FROM ds_",agency,".dc2_i_",networkId," as i
    where i.record_date > '",startPre,"'
    and i.record_date < '",endPre,"'
    and i.advertiser_id in (",advertiserIds,")
    and i.campaign_id in (",campaignIds,")
  ...
  dbSendQuery(redshift_con,paste0("
    drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm;
    ...
    record_year,record_date,paid_search_legacy_keyword_id, 1 as eventind
    ...
    campaign_id, site_id,browser_id, 
    advertiser_id, record_month, record_week, 
    record_year,record_date, '0' as paid_search_legacy_keyword_id ,2 as eventind
  ...
  if(reupload_hierarchy) {
    ...
    dbSendQuery(redshift_con,paste0("
                                    drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_category;
...
if(ppc_match) {
  ...
  colnames(hierarchy) <- gsub("[^a-z_]*","",colnames(hierarchy))
    ...
    hierarchy$sub_channel <- gsub('\\-', '_',
                      gsub('\\+', '', 
                           gsub(' ', '_',
                            gsub("'","",  hierarchy$sub_channel))))
    # need to escape any single quote marks
    hierarchy[,ppc_match_col] <- gsub("'","''",hierarchy[,ppc_match_col])
    ...
    dbSendQuery(redshift_con,
      paste0("
        drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_ppc_categs;
        create table ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_ppc_categs (
  ...
  if(reps>=1) {
    ...
    for(i in 1:reps) {
      ...
      dbSendQuery(redshift_con,
        paste0(" 
          INSERT INTO ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_ppc_categs (
          ppc_match_col
          , sub_channel
          , channel
          ) VALUES (",
  ...
  dbSendQuery(redshift_con,paste0("
  DROP table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged;
  ...
  eventind, record_month, record_week, record_year,record_date
...
dbSendQuery(redshift_con,paste0("
    DROP table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged_ppc;
    CREATE table ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged_ppc as
    ...
    ic.eventind, ic.record_month, ic.record_week, ic.record_year,ic.record_date
    FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm as ic
    left join ds_",agency,".dc2_match_paid_search as d
    on ic.paid_search_legacy_keyword_id=d.paid_search_legacy_keyword_id
    ",toupper(join_type)," JOIN ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_ppc_categs as cat
    ON d.",ppc_match_col,"=cat.ppc_match_col
    where ic.paid_search_legacy_keyword_id <> 0
    and ic.paid_search_legacy_keyword_id <> ''
    "))

# non-ppc clicks
dbSendQuery(redshift_con,paste0("
DROP table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged_nonppc;
CREATE table ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged_nonppc as
SELECT user_id, ic.time, coalesce(channel,'Other') as channel,
coalesce(sub_channel,'Other') as sub_channel, os_id,city_id,browser_id,
eventind, record_month, record_week, record_year,record_date
FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm as ic
...
ON ic.placement_id = cat.placement_id
where ic.paid_search_legacy_keyword_id = 0
or ic.paid_search_legacy_keyword_id = ''
...
from 
(select * from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged_ppc
union all
select * from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged_nonppc) 
"))
  }

  #### remove duplicate events
  dbSendQuery(redshift_con,paste0("DELETE FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged
                                  WHERE row_rank >1
                                  "))
  ...
  dbSendQuery(redshift_con,paste0("
                                  drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_channel_lookup;
  ...
  dbSendQuery(redshift_con,paste0("
                                  drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_filt;
  ...
  dbSendQuery(redshift_con,paste0(" 
                                  drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_filt_LC; 
                                  CREATE TABLE ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_filt_LC AS( 
                                  SELECT s.user_id, s.time,
                                  u.placement_id,u.campaign_id,u.site_id,u.record_month,u.record_week,u.record_year,u.record_date,u.paid_search_legacy_keyword_id
  ...
  dbSendQuery(redshift_con,paste0("
                                  drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_filt_os;
  ...
  dbSendQuery(redshift_con,paste0("
                                  drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_conv;
  ...
  dbSendQuery(redshift_con,paste0("
            drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged2;
  ...
  dbSendQuery(redshift_con,paste0("
                                  drop table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_audience;
...
uni_distr_Q <- dbSendQuery(redshift_con,paste0("
  ...
  uni_distr_Q <- dbSendQuery(redshift_con,paste0("
                                                 select sub_channel,channel,  record_week,
                                                 count(distinct user_id) as uniques
                                                 from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_unique_sub
                                                 group by sub_channel, channel, record_week"))
  uni_distr_sub_week <- fetch(uni_distr_Q, n = -1)
  
  uni_distr_Q <- dbSendQuery(redshift_con,paste0("
                                                 select channel,  record_week, count(distinct user_id) as uniques
                                                 from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_unique_chan
...
# uni_distr_sub_week$record_week <- ifelse(uni_distr_sub_week$record_week>26,
#   paste0(year(startDate),uni_distr_sub_week$record_week),
#     paste0(year(endDate),str_pad(uni_distr_sub_week$record_week,
#                                                       width=2,pad="0")))

# uni_distr_chan_week$record_week <- ifelse(uni_distr_chan_week$record_week>26,
#   paste0(year(startDate),uni_distr_chan_week$record_week),
#     paste0(year(endDate),str_pad(uni_distr_chan_week$record_week,
#                                                       width=2,pad="0")))
...
dbSendQuery(redshift_con,paste0("
    DROP table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_lc_merged_ppc;
    CREATE table ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_lc_merged_ppc as
    SELECT ic.user_id, ic.time, coalesce(cat.channel,'Other') as channel,
    coalesce(cat.sub_channel,'Other') as sub_channel, ic.record_month, ic.record_week, ic.record_year,ic.record_date
    FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_filt_lc as ic
    left join ds_",agency,".dc2_match_paid_search as d
    on ic.paid_search_legacy_keyword_id=d.paid_search_legacy_keyword_id
    ",toupper(join_type)," JOIN ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_ppc_categs as cat
    ON d.",ppc_match_col,"=cat.ppc_match_col
...
# non-ppc clicks
dbSendQuery(redshift_con,paste0("
DROP table if exists ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_lc_merged_nonppc;
CREATE table ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_lc_merged_nonppc as
SELECT user_id, ic.time, coalesce(channel,'Other') as channel,
coalesce(sub_channel,'Other') as sub_channel, record_month, record_week, record_year,record_date
FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_filt_lc as ic
",toupper(join_type)," JOIN ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_category as cat
ON ic.placement_id = cat.placement_id
where ic.paid_search_legacy_keyword_id = 0
...
row_number() over(partition by user_id,sub_channel,time order by user_id) as row_rank
from 
(select * from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_lc_merged_ppc
union all
select * from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_lc_merged_nonppc) 
"))
...
############################
  ...
  dups <- duplicated(lc_na[,c("user_id","time")])
  if(sum(dups)>0) {
    lc_na <- lc_na[-which(dups),]}
  
  lc_na$lc <- 1
  lc_na$record_month <- paste0(lc_na$record_year,str_pad(lc_na$record_month,width=2,pad="0"))
  lc_na$record_week  <- gsub("-W","",ISOweek(lc_na$record_date))
...
dbSendQuery(redshift_con,paste0("
  ...
  event_dow_distr$record_week <- gsub("-W","",ISOweek(event_dow_distr$date))
  ...
  aud_distr_Q <- dbSendQuery(redshift_con,paste0("
                                                 select sub_channel, record_week, count(distinct user_id) as audience
                                                 from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged2
                                                 group by sub_channel, record_week"))
  aud_distr_sub_week <- fetch(aud_distr_Q, n = -1)
  
  aud_distr_Q <- dbSendQuery(redshift_con,paste0("
                                                 select channel, record_week, count(distinct user_id) as audience
                                                 from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged2
                                                 group by channel, record_week"))
...
# aud_distr_sub_week$record_week <- ifelse(aud_distr_sub_week$record_week>26,
#   paste0(year(startDate),aud_distr_sub_week$record_week),
#     paste0(year(endDate),str_pad(aud_distr_sub_week$record_week,
#                                                       width=2,pad="0")))

# aud_distr_chan_week$record_week <- ifelse(aud_distr_chan_week$record_week>26,
#   paste0(year(startDate),aud_distr_chan_week$record_week),
#     paste0(year(endDate),str_pad(aud_distr_chan_week$record_week,
#                                                       width=2,pad="0")))
...
dbSendQuery(redshift_con,paste0("
                                  ...
                                  record_year,record_date,row_rank
...
icM_20 <- unique(icM_20)
icM_conv_sec_filt <- unique(icM_conv_sec_filt)
