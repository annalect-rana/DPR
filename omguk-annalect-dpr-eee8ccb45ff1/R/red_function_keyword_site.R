...
red_function_keyword_site <- function(username,pw,DPID,dbname="annalectuk",dbHost="54.77.118.78",
                         agency,networkId,advertiserIds,campaignIds,activity_sub_type, activity_type=NULL, eventid.ign=NULL, 
...
if(length(tables_to_remove)>0) {
  ...
  dbSendQuery(redshift_con,paste0("
    ...
    select user_id, min(s.time) as time, activity_sub_type, os_id,browser_id, keyword_id,
    page_id, buy_id, site_id, advertiser_id, record_month, record_week, record_year, revenue as initial_revenue
...
group by user_id,  activity_sub_type, os_id,browser_id, keyword_id,
...
# calc for the revenue across all conversion types now:
 dbSendQuery(redshift_con,paste0("
    ...
    select user_id, sum(revenue) as all_revenue
    from ds_",agency,".dc_na_",networkId," as s
    where record_date > '",startDate,"'
    and record_date < '",endDate,"'
    and advertiser_id in (",advertiserIds,")
...
group by user_id"))
  ...
  SELECT user_id, c1.time, os_id, city_id,page_id, buy_id,
  site_id,browser_id,
  advertiser_id, record_month, record_week, record_year,keyword_id
  ...
  and user_id not in (
    select e.user_id from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_pre_na_UIDs as e)
  UNION ALL
  SELECT c.user_id, c.time, c.os_id,c.city_id, c.page_id,
  c.buy_id,c.site_id,c.browser_id, 
   c.advertiser_id, c.record_month,
  c.record_week, c.record_year,c.keyword_id
  ...
  dbSendQuery(redshift_con,paste0("
                                  ...
                                  SELECT user_id, i1.time,city_id, os_id,
                                   page_id, buy_id,browser_id,
                                  site_id, advertiser_id, record_month,
    ...
    select e.user_id from ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_pre_na_UIDs as e)
                                  UNION ALL
                                  SELECT i.user_id, i.time, i.city_id, i.os_id, 
                                  i.page_id,i.buy_id, i.browser_id,
                                  i.site_id, i.advertiser_id, i.record_month, 
  ...
  if(reupload_hierarchy) {
    ...
    if(reps>=1) {
      ...
      for(i in 1:reps) {
        ...
        dbSendQuery(redshift_con,paste0(" 
                                        INSERT INTO ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_category (page_id
                                        , buy_id
                                        , site_id
                                        , channel
                                        , sub_channel
                                  ...
                                  do.call(paste0,list(lapply((i-1)*n+1:n,function(k) {
                                    paste0(paste0(hierarchy[k,c("page_id","buy_id","site_id")],collapse=", "), 
                                           ", '",
    ...
    dbSendQuery(redshift_con,paste0(" 
      ...
      do.call(paste0,list(lapply((reps*n+1):N,function(i) {
        paste0(paste0(hierarchy[i,c("page_id","buy_id","site_id")],collapse=", "), 
               ", '",
...
dbSendQuery(redshift_con,paste0("
    ...
    SELECT ic.user_id, ic.time, coalesce(cat.channel,'Paid_Search') as channel,
    ...
    FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm as ic
    join ds_",agency,".dc_match_dartsearch as d
...
###### CUSTOM CODE TO REMOVE CERTAIN SUB CHANNELS
dbSendQuery(redshift_con,paste0("
                                DELETE FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_conv
                                WHERE sub_channel in ('",paste0(subs_to_remove,collapse=", "),"');"))
...
###### CUSTOM CODE TO REMOVE CERTAIN SUB CHANNELS
dbSendQuery(redshift_con,paste0("
                                  DELETE FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_icm_merged2
                                  WHERE sub_channel in ('",paste0(subs_to_remove,collapse=", "),"');"))
...
############ END OF UNIQUES
  ...
  if(!ppc_match) {
    ...
    dbSendQuery(redshift_con,paste0("
      ...
      SELECT user_id, lc.time, coalesce(cat.channel,'Paid_Search') as channel,
       coalesce(cat.sub_channel,'Other') as sub_channel,
      record_month,record_week,record_year
      FROM ds_",agency,".","DP",date_code,"_",analyst_code,"_",DPID,"_na_filt_LC as lc
      left join ds_",agency,".dc_match_dartsearch as d
