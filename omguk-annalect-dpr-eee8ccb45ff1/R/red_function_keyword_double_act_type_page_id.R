...
red_function_keyword_double_act_type_page_id <- function(username,pw,DPID,dbname="annalectuk",dbHost="54.77.118.78",
...
failure <- checks_function_keyword_OMD(startDate,endDate,
                                   startPre,endPre,
                                   reupload_hierarchy,
                                   ppc_match,
                                   hierarchy_location,
...
if(ppc_match) {
  ...
  colnames(hierarchy) <- gsub("[^a-z_]*","",colnames(hierarchy))
    ...
    hierarchy <- hierarchy[,c(ppc_match_col,"page_id","sub_channel","channel")]
